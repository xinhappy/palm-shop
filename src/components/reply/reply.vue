<template>
  <div>
    <x-header style="background-color:#fff" :left-options="{backText: '',showBack: false}">
      ...
      <a style="color: #333">复机申请</a>
    </x-header>
    <div style="margin-top: 2vw">
      <tab :line-width="1" custom-bar-width="60px" active-color="#329dff">
        <tab-item selected @on-item-click="go('telephone-r')">固话账户</tab-item>
        <tab-item @on-item-click="go('broadband-r')">宽带账户</tab-item>
        <tab-item @on-item-click="go('tv-r')">电视账户</tab-item>
      </tab>
    </div>
    <div>
